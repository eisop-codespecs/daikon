...
if [ ! -d ../fjalar ] ; then
  if [ -d "/tmp/$USER/git-scripts" ] ; then
    ...
    mkdir -p "/tmp/$USER"
    (cd "/tmp/$USER" && (git clone --depth 1 -q https://github.com/eisop-plume-lib/git-scripts.git || (sleep 1m && git clone --depth 1 -q https://github.com/eisop-plume-lib/git-scripts.git)))
  fi
