...
if [ -d "/tmp/$USER/plume-scripts" ] ; then
  ...
  mkdir -p "/tmp/$USER" && (cd "/tmp/$USER" && (git clone --depth 1 -q https://github.com/eisop-plume-lib/plume-scripts.git || (sleep 1m && git clone --depth 1 -q https://github.com/eisop-plume-lib/plume-scripts.git)))
fi
