...
if [[ "${GROUP}" == "misc" || "${GROUP}" == "all" ]]; then
  ...
  if [ -d "/tmp/plume-scripts" ] ; then
    ...
    (cd /tmp && (git clone --filter=blob:none -q https://github.com/eisop-plume-lib/plume-scripts.git || (sleep 1m && git clone --filter=blob:none -q https://github.com/eisop-plume-lib/plume-scripts.git)))
  fi
